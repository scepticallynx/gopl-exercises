...
the primary sort key is the most recently clicked column head, the secondary sort
key is the second-most recently clicked column head, and so on. Define an
implementation of sort.Interface for use by such table.
*/
package main
...
func main() {
	printTracks(tracks)
	sort.Sort(MultiTierSort(tracks, []string{"Length", "Year", "Title"}))
	printTracks(tracks)
}
